...
#'
#' @seealso \code{\link{tolerance_svd}}, \code{\link{matrix.exponent}}, and \code{\link{matrix.generalized.inverse}}
#'
#' @author Derek Beaton
#'
...
## TODO: rebuild with gsvd() not tolerance_svd() [though technically it's just a pass through at that point...]
...
matrix.low.rank.rebuild <- function(x, k = 0, ...){
  ...
  ## quick tests for escape
    ### these should probably exist elsewhere, too. in those cases it should silently work or fail. TBD.
  if( !is.numeric(k) ){
  ...
  if( length(k) > 1 ){
    ...
    res <- tolerance_svd(x, nu = max(comps.to.rebuild), nv = max(comps.to.rebuild), ...)
    ## get rid of sweep()
    ...
    ## this rebuilds based on first K components where K = round(k)
      ...
      res <- tolerance_svd(x, nu = round(k), nv = round(k) )
